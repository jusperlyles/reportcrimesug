...
const CARDS = [
  ...
  { key: "get-help", label: "Get Help", desc: "Contact support & AI assistant", route: "/get-help", icon: FaHandsHelping, color: "hsl(142 71% 45%)" },
  ...
  { key: "missing-persons", label: "Missing Persons", desc: "View & report missing people", route: "/missing-persons", icon: FaUserFriends, color: "hsl(217 91% 60%)" },
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="relative mb-6">
        <FaSearch className="absolute left-4 top-1/2 -translate-y-1/2 text-muted-foreground" size={14} />
        <input
          type="text"
          placeholder="Search features..."
          value={search}
          onChange={(e) => setSearch(e.target.value)}
          className="w-full rounded-full py-2.5 pl-10 pr-10 border-2 border-primary/20 bg-card text-foreground text-sm focus:outline-none focus:border-primary focus:shadow-[0_0_0_3px_hsl(234_85%_65%_/_0.12)] transition-all"
        />
        {search && (
          <button
            onClick={() => setSearch("")}
        ...
        {filtered.map((card, i) => {
          ...
          return (
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              className="group relative overflow-hidden rounded-2xl border border-border/50 bg-card backdrop-blur-sm text-left p-4 transition-shadow duration-300 hover:shadow-[0_16px_48px_hsl(234_85%_65%_/_0.22)] hover:border-primary/30"
            >
              {/* Icon */}
              <div
              ...
              <h3 className="font-semibold text-sm text-foreground leading-tight mb-1">{card.label}</h3>
